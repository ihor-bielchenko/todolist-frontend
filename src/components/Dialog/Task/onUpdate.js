...
import { fireSchema as actionTaskSchema } from 'components/Store/task/actions/schema.js';import utilsValidateEmail from 'utils/validateEmail.js';

export const onUpdate = async (taskId, {
	name,
	email,
	text,
}, onDialogClose) => {
	...
	try {
		await axios.patch(`${process.env.URL_API}/${process.env.URL_API_TASK}/${taskId}`, {
			name,
			email,
			text,
		});
		await actionTaskSchema()();
		actionSnackbarShow('success', 'Задача успешно изменена')();
		onDialogClose();
	}
	catch (err) {
